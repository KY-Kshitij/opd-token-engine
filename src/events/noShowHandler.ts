...
/**
 * Handle patient no-show event
 * 
 * State transition: ALLOCATED → NO_SHOW
 * 
 * Side effects:
...
export function handleNoShow(
    ...
    if (token.status !== TokenStatus.ALLOCATED) {
        throw new Error(`Cannot mark no-show for token ${token.id} with status ${token.status}`);
    }
